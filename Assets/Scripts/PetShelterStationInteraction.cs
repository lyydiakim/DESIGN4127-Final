...
/// <summary>
/// Pet shelter minigame: Fire 1 opens the overlay. While open, Fire 1 / Fire 2 = volunteer shifts (energy to network),
/// Fire 4 (Y) closes.
/// </summary>
[RequireComponent(typeof(Collider2D))]
public class PetShelterStationInteraction : MonoBehaviour
{
    ...
    [Header("Volunteer — A (Fire 1 when menu open)")]
    [SerializeField] int optionAEnergyCost = 15;
    [SerializeField] int optionANetworkReward = 1;

    [Header("Volunteer — B (Fire 2 when menu open)")]
    [SerializeField] int optionBEnergyCost = 15;
    [SerializeField] int optionBNetworkReward = 1;

    [SerializeField] string stationTitle = "Pet Shelter";

    private sealed class ShelterHooks
    {
    ...
    private readonly Dictionary<playerController, ShelterHooks> _hooks = new();
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (bank == null)
        {
            Debug.LogError($"{name}: PetShelterStationInteraction has no ResourceBank assigned.", this);
            return;
        }

        Debug.Log($"{name}: player {pc.name} entered pet shelter zone (station: {stationTitle}).", this);

        _menuOpen[pc] = false;

        var hook = new ShelterHooks();
        ...
        {
            Debug.LogError(
                "PetShelterStationInteraction: PlayerTransactionFeedback not found in scene. Add it under your UI Canvas (player boards).",
                this);
            return;
        }
    ...
    private void TryWork(playerController pc, int energyCost, int networkReward)
    {
        ...
        {
            Debug.LogError($"{name}: Assign Energy and Network resources on PetShelterStationInteraction.", this);
            return;
        }
