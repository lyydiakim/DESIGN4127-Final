...
public class ParkStationInteraction : MonoBehaviour
{
    ...
    [Header("Trade (A or B when menu is open)")]
    [SerializeField] int energyCost = 10;
    [SerializeField] int networkReward = 1;

    [SerializeField] string stationTitle = "Park";

    private sealed class ParkHooks
    {
    ...
    private readonly Dictionary<playerController, ParkHooks> _hooks = new();
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (bank == null)
        {
            Debug.LogError($"{name}: ParkStationInteraction has no ResourceBank assigned.", this);
            return;
        }

        Debug.Log($"{name}: player {pc.name} entered park zone (station: {stationTitle}).", this);

        _menuOpen[pc] = false;

        var hook = new ParkHooks();
        ...
        {
            Debug.LogError(
                "ParkStationInteraction: PlayerTransactionFeedback not found in scene. Add it under your UI Canvas (player boards).",
                this);
            return;
        }
    ...
    private void TryEnergyForNetwork(playerController pc)
    {
        if (energyResource == null || networkResource == null)
        {
            Debug.LogError($"{name}: Assign Energy and Network (Friendship) resources on ParkStationInteraction.", this);
            return;
        }
