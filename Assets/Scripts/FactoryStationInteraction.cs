...
public class FactoryStationInteraction : MonoBehaviour
{
    ...
    [Header("Work from office — A (Fire 1 when menu open)")]
    [SerializeField] int optionAEnergyCost = 15;
    [SerializeField] int optionACoinReward = 60;

    [Header("Work from office — B (Fire 2 when menu open)")]
    [SerializeField] int optionBEnergyCost = 20;
    [SerializeField] int optionBCoinReward = 75;

    [SerializeField] string stationTitle = "Factory";

    private sealed class FactoryHooks
    {
    ...
    private readonly Dictionary<playerController, FactoryHooks> _hooks = new();
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (bank == null)
        {
            Debug.LogError($"{name}: FactoryStationInteraction has no ResourceBank assigned.", this);
            return;
        }

        Debug.Log($"{name}: player {pc.name} entered factory zone (station: {stationTitle}).", this);

        _menuOpen[pc] = false;

        var hook = new FactoryHooks();
        ...
        {
            Debug.LogError(
                "FactoryStationInteraction: PlayerTransactionFeedback not found in scene. Add it under your UI Canvas (player boards).",
                this);
            return;
        }
    ...
    private void OnPressFire1(playerController pc)
    {
        ...
        TryWork(pc, optionAEnergyCost, optionACoinReward);
    }

    private void OnPressFire2(playerController pc)
    {
        if (pc == null || !MenuIsOpen(pc)) return;
        TryWork(pc, optionBEnergyCost, optionBCoinReward);
    }
    ...
    private void TryWork(playerController pc, int energyCost, int coinReward)
    {
        if (energyResource == null || moneyResource == null)
        {
            Debug.LogError($"{name}: Assign Energy and Money resources on FactoryStationInteraction.", this);
            return;
        }
