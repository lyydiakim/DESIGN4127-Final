...
/// <summary>
/// Deli minigame: Fire 1 opens the overlay. While open, Fire 1 = order option A (coins for energy + network),
/// Fire 2 = option B, Fire 4 (Y) closes. Costs are paid in coins (Money resource).
/// </summary>
[RequireComponent(typeof(Collider2D))]
public class DeliStationInteraction : MonoBehaviour
{
    ...
    [Header("Order at counter — A (Fire 1 when menu open)")]
    [SerializeField] int optionACoinCost = 15;
    [SerializeField] int optionAEnergyReward = 12;
    [SerializeField] int optionANetworkReward = 1;

    [Header("Order at counter — B (Fire 2 when menu open)")]
    [SerializeField] int optionBCoinCost = 20;
    [SerializeField] int optionBEnergyReward = 17;
    [SerializeField] int optionBNetworkReward = 1;

    [SerializeField] string stationTitle = "Deli";

    private sealed class DeliHooks
    {
    ...
    private readonly Dictionary<playerController, DeliHooks> _hooks = new();
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (bank == null)
        {
            Debug.LogError($"{name}: DeliStationInteraction has no ResourceBank assigned.", this);
            return;
        }

        Debug.Log($"{name}: player {pc.name} entered deli zone (station: {stationTitle}).", this);

        _menuOpen[pc] = false;

        var hook = new DeliHooks();
        ...
        {
            Debug.LogError(
                "DeliStationInteraction: PlayerTransactionFeedback not found in scene. Add it under your UI Canvas (player boards).",
                this);
            return;
        }
    ...
    private void OnPressFire1(playerController pc)
    {
        ...
        TryOrder(pc, optionACoinCost, optionAEnergyReward, optionANetworkReward);
    }

    private void OnPressFire2(playerController pc)
    {
        if (pc == null || !MenuIsOpen(pc)) return;
        TryOrder(pc, optionBCoinCost, optionBEnergyReward, optionBNetworkReward);
    }
    ...
    private void TryOrder(playerController pc, int coinCost, int energyReward, int networkReward)
    {
        if (moneyResource == null || energyResource == null || networkResource == null)
        {
            Debug.LogError($"{name}: Assign Money, Energy, and Network resources on DeliStationInteraction.", this);
            return;
        }
        ...
        {
            new ResourceCost { resource = energyResource, amount = energyReward },
    ...
    private string BoardLineClosedPrompt(playerController pc)
    {
        ...
        if (pi != null && pi.actions != null)
        {
            ...
            return $"<b>{stationTitle}</b>\n{open} Order at the Counter";
        }

        if (PlayerResourceBindingPrompts.IsKeyboardP2Player(pc))
            return $"<b>{stationTitle}</b>\n<b>[E]</b> or <b>[Insert]</b> Order at the Counter";
        return $"<b>{stationTitle}</b>\n[A] / [E] Order at the Counter";
    }
